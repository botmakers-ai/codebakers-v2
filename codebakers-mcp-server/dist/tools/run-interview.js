...
async function generateInterviewResult(description, context) {
    // Infer project type
    ...
    const userFlows = [];
    if (projectType === 'email') {
        userFlows.push({ name: 'View inbox', description: 'User sees list of messages', steps: ['Load messages', 'Display list', 'Show unread count'], priority: 'P0', status: 'pending' }, { name: 'Read message', description: 'User opens and reads message', steps: ['Click message', 'Load content', 'Mark as read'], priority: 'P0', status: 'pending' }, { name: 'Send message', description: 'User composes and sends message', steps: ['Open composer', 'Enter recipients', 'Write message', 'Send'], priority: 'P0', status: 'pending' }, { name: 'Delete message', description: 'User deletes message', steps: ['Select message', 'Click delete', 'Confirm', 'Update UI'], priority: 'P1', status: 'pending' }, { name: 'Search messages', description: 'User searches for messages', steps: ['Enter query', 'Execute search', 'Display results'], priority: 'P1', status: 'pending' });
    }
    else {
        userFlows.push({ name: 'User login', description: 'User authenticates', steps: ['Enter credentials', 'Submit', 'Redirect to app'], priority: 'P0', status: 'pending' }, { name: 'View list', description: 'User sees list of items', steps: ['Load items', 'Display list'], priority: 'P0', status: 'pending' }, { name: 'Create item', description: 'User creates new item', steps: ['Open form', 'Fill fields', 'Submit', 'Update list'], priority: 'P0', status: 'pending' });
    }
    // External services
...
async function writeCredentialsNeeded(cwd, result) {
...
function generateInterviewReport(result) {
...
//# sourceMappingURL=run-interview.js.map