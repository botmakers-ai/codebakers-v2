...
export async function validateMockups(args) {
 ...
 */
async function validateLayoutIssues(file, content, result) {
    ...
    for (let i = 0; i < absoluteElements.length; i++) {
        for (let j = i + 1; j < absoluteElements.length; j++) {
            ...
            const overlaps = !(a.left + a.width < b.left ||
 ...
 */
async function validateViewportIssues(file, content, result) {
 ...
 */
async function validateStates(files, result) {
    ...
    for (const file of files) {
        ...
        if (file.includes('loading'))
            componentFiles.get(baseName).add('loading');
        if (file.includes('error'))
            componentFiles.get(baseName).add('error');
        if (file.includes('empty'))
            componentFiles.get(baseName).add('empty');
 ...
 */
async function validateResponsive(files, result) {
 ...
 */
async function validateDesignTokens(mockupPath, files, result) {
 ...
 */
async function validateAccessibility(file, content, result) {
 ...
 */
function generateValidationReport(result, fileCount) {
    ...
    if (result.total_critical > 0) {
        ...
        for (const issue of result.viewport_issues.filter(i => i.severity === 'critical')) {
            ...
            if (issue.file !== 'N/A') {
                ...
                if (issue.overflow)
                    report += `**Overflow:** ${issue.overflow}px\n`;
            }
            report += `**Issue:** ${issue.description}\n`;
            report += `**Fix available:** ${issue.fix_available ? 'Yes (auto-fix)' : 'Manual'}\n\n`;
        }
...
//# sourceMappingURL=validate-mockups.js.map