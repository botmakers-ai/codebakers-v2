...
interface FilterParameter {
  ...
  filter_type: 'search' | 'sort' | 'filter';
  values?: string[];
}
...
interface Relationship {
  from_entity: string;
  to_entity: string;
  relationship_type: '1:1' | '1:many' | 'many:many';
  implied_by: string; // What UI element implies this relationship
}
...
interface MockupAnalysis {
  ...
  state_machines: StateValue[];
  computed_values: DataField[];
  notes: string[];
}

export async function analyzeMockupsDeep(args: { mockup_folder?: string }): Promise<string> {
 ...
 */
async function analyzeSingleMockup(filename: string, content: string): Promise<MockupAnalysis> {
 ...
 */
function generateMockAnalysisDocument(analyses: MockupAnalysis[]): string {
 ...
 */
function generateAnalysisSummary(analyses: MockupAnalysis[]): string {
  const totalFields = analyses.reduce((sum, a) => sum + a.data_fields.length, 0);
  const totalInputs = analyses.reduce((sum, a) => sum + a.form_inputs.length, 0);
  const totalRelationships = analyses.reduce((sum, a) => sum + a.relationships.length, 0);
...
function inferDataType(fieldName: string, context: string): string {
  const lowerField = fieldName.toLowerCase();

  if (lowerField.includes('date') || lowerField.includes('created') || lowerField.includes('updated')) return 'timestamp';
  ...
  if (lowerField.includes('id') || lowerField.endsWith('_id')) return 'uuid';
  if (lowerField.includes('count') || lowerField.includes('total') || lowerField.includes('amount')) return 'number';
  if (lowerField.includes('is_') || lowerField.includes('has_')) return 'boolean';
  if (lowerField.includes('price') || lowerField.includes('cost')) return 'decimal';

  return 'text';
}

function inferEntity(fieldName: string, context: string): string {
  ...
  if (context.includes('order.')) return 'order';
  if (context.includes('product.')) return 'product';

  return 'unknown';
}

function isComputedField(fieldName: string): boolean {
...
function inferComputationRule(fieldName: string): string {
  const lower = fieldName.toLowerCase();
  if (lower.includes('total')) return 'SUM of related values';
  if (lower.includes('count')) return 'COUNT of related items';
  if (lower.includes('average')) return 'AVERAGE of related values';
  if (lower.includes('length')) return 'COUNT of array items';
  return 'Derived value';
}

function extractValidationRules(inputName: string, context: string): string[] {
  const rules: string[] = [];

  if (context.includes(`required`) && context.includes(inputName)) rules.push('required');
  if (context.includes(`minLength`) && context.includes(inputName)) rules.push('minLength');
  if (context.includes(`maxLength`) && context.includes(inputName)) rules.push('maxLength');
  if (context.includes(`pattern`) && context.includes(inputName)) rules.push('pattern');
  if (context.includes(`email`) && context.includes(inputName)) rules.push('email format');

  return rules;
}

function inferFilterType(matchText: string): 'search' | 'sort' | 'filter' {
  if (matchText.toLowerCase().includes('search')) return 'search';
  if (matchText.toLowerCase().includes('sort')) return 'sort';
  return 'filter';
}

function extractFilterValues(filterName: string, context: string): string[] | undefined {
  // This would require more sophisticated parsing
  // For now, return undefined to indicate dynamic values
  return undefined;
}

function extractStateValues(content: string): string[] {
