...
interface BuildArgs {
  ...
  skip_quality_gates?: boolean; // Skip quality gates (default: false)
  skip_deploy?: boolean; // Skip deployment (default: false)
  skip_docs?: boolean; // Skip documentation (default: false)
...
async function runQualityGates(cwd: string, skipDeploy: boolean, skipDocs: boolean, skipChatbot: boolean): Promise<any> {
  const gates: any = {
    accessibility: null,
    performance: null,
    security: null,
...
function generateBuildReport(results: any[], total: number, qualityGates: any = null): string {
