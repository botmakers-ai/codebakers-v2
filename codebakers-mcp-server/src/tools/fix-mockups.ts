...
interface FixResult {
  auto_fixed: FixedIssue[];
  manual_review: ManualIssue[];
...
export async function fixMockups(args: { mockup_folder?: string }): Promise<string> {
 ...
 */
async function fixOverlappingContent(
  mockupPath: string,
  files: string[],
  result: FixResult
): Promise<void> {
  for (const file of files) {
    ...
    const elements: { id: string; left: number; top: number; width: number; index: number }[] = [];
      ...
      for (let j = i + 1; j < elements.length; j++) {
        ...
        if (Math.abs(a.left - b.left) < 20 && Math.abs(a.top - b.top) < 20) {
          ...
          content = content.replace(
            styleMatches[b.index][0],
            styleMatches[b.index][0].replace(/left:\s*\d+/, `left: ${newLeft}`)
          );
          modified = true;
 ...
 * Fix out of viewport elements
 */
async function fixViewportIssues(
  mockupPath: string,
  files: string[],
  result: FixResult
): Promise<void> {
 ...
 */
async function generateMobileMockups(
  mockupPath: string,
  files: string[],
  result: FixResult
): Promise<void> {
  ...
  if (!hasMobile) {
    ...
    for (const file of files) {
      if (file.includes('mobile') || file.includes('375')) continue;
      ...
      mobileContent = mobileContent.replace(
        /display:\s*flex;?\s*flex-direction:\s*row/g,
 ...
 */
async function generateMissingStates(
  mockupPath: string,
  files: string[],
  result: FixResult
): Promise<void> {
  ...
  for (const file of files) {
    ...
    if (file.includes('loading')) components.get(baseName)!.add('loading');
    if (file.includes('error')) components.get(baseName)!.add('error');
    if (file.includes('empty')) components.get(baseName)!.add('empty');
  }
...
function generateLoadingState(component: string): string {
  return `<!DOCTYPE html>
<html>
<head>
...
function generateErrorState(component: string): string {
  return `<!DOCTYPE html>
<html>
<head>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
function generateEmptyState(component: string): string {
  return `<!DOCTYPE html>
<html>
<head>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
 ...
 */
async function standardizeColors(
  mockupPath: string,
  files: string[],
  result: FixResult
): Promise<void> {
 ...
 * Generate fix report
 */
function generateFixReport(result: FixResult): string {
  ...
  if (result.auto_fixed.length > 0) {
    ...
    const byType = new Map<string, FixedIssue[]>();
    for (const fix of result.auto_fixed) {
      if (!byType.has(fix.type)) byType.set(fix.type, []);
