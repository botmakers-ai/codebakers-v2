...
interface ReadDependency {
  component: string;
  reads_from_stores: string[];
  queries_entities: string[];
...
interface StoreConnection {
  store_name: string;
  sourced_from_entities: string[];
  connected_to_stores: string[];
...
interface DependencyMap {
  read_dependencies: ReadDependency[];
  write_dependencies: WriteDependency[];
  store_connections: StoreConnection[];
  cascade_effects: CascadeEffect[];
  critical_paths: string[];
}

export async function mapDependencies(args: { schema_file?: string; analysis_file?: string }): Promise<string> {
 ...
 */
function buildComprehensiveDependencyMap(schema: string, analysis: string): DependencyMap {
  ...
  for (const screen of screens) {
    if (screen.form_inputs.length > 0) {
      ...
      if (entity && entity !== 'unknown') {
        const writeDep: WriteDependency = {
  ...
  for (const entity of entities) {
    const storeConn: StoreConnection = {
  ...
  for (const writeDep of map.write_dependencies) {
    ...
    if (totalStores >= 3) {
      map.critical_paths.push(
        `${writeDep.mutation} affects ${totalStores} stores: ${[...writeDep.must_update_stores, ...writeDep.cascade_updates].join(', ')}`
      );
    }
  }
 ...
 * Extract entities from schema
 */
function extractEntitiesFromSchema(schema: string): Array<{ name: string; foreign_keys: Array<{ column_name: string; references_table: string }> }> {
  const entities: Array<{ name: string; foreign_keys: Array<{ column_name: string; references_table: string }> }> = [];
  ...
  while ((match = tableRegex.exec(schema)) !== null) {
    ...
    const foreign_keys: Array<{ column_name: string; references_table: string }> = [];
 ...
 */
function extractScreensFromAnalysis(analysis: string): Array<{ name: string; data_fields: Array<{ field_name: string; source_entity: string }>; form_inputs: Array<{ input_name: string; target_entity: string }>; filters: Array<{ filter_name: string; filter_type: string }> }> {
  const screens: Array<{ name: string; data_fields: Array<{ field_name: string; source_entity: string }>; form_inputs: Array<{ input_name: string; target_entity: string }>; filters: Array<{ filter_name: string; filter_type: string }> }> = [];
...
function extractDataFieldsForScreen(analysis: string, screenName: string): Array<{ field_name: string; source_entity: string }> {
  const fields: Array<{ field_name: string; source_entity: string }> = [];

  // Find the screen section
  const screenSection = extractScreenSection(analysis, screenName);
  if (!screenSection) return fields;
...
function extractFormInputsForScreen(analysis: string, screenName: string): Array<{ input_name: string; target_entity: string }> {
  const inputs: Array<{ input_name: string; target_entity: string }> = [];

  const screenSection = extractScreenSection(analysis, screenName);
  if (!screenSection) return inputs;
...
function extractFiltersForScreen(analysis: string, screenName: string): Array<{ filter_name: string; filter_type: string }> {
  const filters: Array<{ filter_name: string; filter_type: string }> = [];

  const screenSection = extractScreenSection(analysis, screenName);
  if (!screenSection) return filters;
...
function extractScreenSection(analysis: string, screenName: string): string | null {
 ...
 */
function findCascadeEffects(
  entity: string,
  allEntities: Array<{ name: string; foreign_keys: Array<{ column_name: string; references_table: string }> }>,
  allScreens: Array<{ name: string; data_fields: Array<{ field_name: string; source_entity: string }>; form_inputs: any; filters: any }>
): { affected_stores: string[]; affected_components: string[] } {
  const affected_stores: string[] = [];
  const affected_components: string[] = [];
 ...
 */
function generateDependencyMapDocument(map: DependencyMap): string {
 ...
 */
function generateDependencyMapSummary(map: DependencyMap): string {
