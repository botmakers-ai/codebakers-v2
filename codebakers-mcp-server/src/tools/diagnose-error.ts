...
interface DiagnoseArgs {
  ...
  stack_trace?: string;
  context?: string; // Additional context about what user was doing
}
...
export async function diagnoseError(args: DiagnoseArgs): Promise<string> {
...
async function findRelatedErrors(cwd: string, errorMessage: string): Promise<string[]> {
...
function extractKeyTerms(error: string): string[] {
  const terms: string[] = [];
  ...
  if (error.includes('.tsx')) terms.push('tsx');
  if (error.includes('.ts')) terms.push('typescript');
  if (error.includes('.jsx')) terms.push('jsx');

  return terms;
}

function extractAffectedFiles(stackTrace: string): string[] {
  const files: string[] = [];
...
function analyzeRootCause(error: string, stackTrace: string, context?: string): { cause: string; confidence: 'high' | 'medium' | 'low' } {
...
function generateFixApproaches(error: string, rootCause: { cause: string }, affectedFiles: string[]): FixApproach[] {
...
async function logError(cwd: string, error: string, stackTrace: string, diagnosis: DiagnosisResult): Promise<void> {
  const errorLogPath = path.join(cwd, '.codebakers', 'ERROR-LOG.md');
...
function generateDiagnosisReport(diagnosis: DiagnosisResult, error: string): string {
