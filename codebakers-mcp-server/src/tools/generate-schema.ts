...
export async function generateSchema(args: { analysis_file?: string }): Promise<string> {
 ...
 */
function parseAnalysisForSchema(analysisContent: string): { tables: Table[]; enums: EnumType[] } {
  const tables: Table[] = [];
  const enums: EnumType[] = [];
  const entityFields = new Map<string, Set<string>>();
  const entityTypes = new Map<string, Map<string, string>>();
 ...
 */
function generateSQLSchema(tables: Table[], enums: EnumType[]): string {
  ...
  for (const table of tables) {
    ...
    for (const col of table.columns) {
      ...
      if (col.unique && !col.is_primary_key) def += ' UNIQUE';
      columnDefs.push(def);
    }
 ...
 */
function generateSchemaSummary(tables: Table[], enums: EnumType[]): string {
...
function mapToPostgreSQLType(dataType: string): string {
...
function mapInputTypeToSQLType(inputType: string): string {
  const typeMap: Record<string, string> = {
    'text': 'text',
    'email': 'email',
    'password': 'text',
