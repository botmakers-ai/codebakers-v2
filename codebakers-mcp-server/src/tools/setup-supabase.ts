...
export async function setupSupabase(args: SupabaseSetupArgs = {}): Promise<string> {
...
async function configureExistingProject(cwd: string, config: {
  supabase_url: string;
  supabase_anon_key: string;
  supabase_service_key?: string;
  database_url?: string;
...
async function createNewProject(cwd: string, projectName: string, region: string): Promise<string> {
...
async function startOAuthFlow(): Promise<SupabaseCredentials> {
...
async function exchangeCodeForToken(code: string): Promise<SupabaseCredentials> {
...
async function createSupabaseProject(credentials: SupabaseCredentials, name: string, region: string): Promise<any> {
...
async function getOrganizationId(credentials: SupabaseCredentials): Promise<string> {
...
async function waitForProjectReady(credentials: SupabaseCredentials, projectId: string): Promise<void> {
...
async function getProjectDetails(credentials: SupabaseCredentials, projectId: string): Promise<SupabaseProject> {
  const response = await fetch(`${SUPABASE_AUTH_URL}/v1/projects/${projectId}`, {
    headers: {
      'Authorization': `Bearer ${credentials.access_token}`
    }
...
async function saveCredentials(cwd: string, credentials: SupabaseCredentials): Promise<void> {
