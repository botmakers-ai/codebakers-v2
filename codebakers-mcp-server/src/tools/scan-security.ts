...
interface SecurityArgs {
  block_on_critical?: boolean; // Block deployment if critical found, default: true
}
...
export async function scanSecurity(args: SecurityArgs = {}): Promise<string> {
...
async function scanDependencies(cwd: string): Promise<SecurityIssue[]> {
  const issues: SecurityIssue[] = [];
  ...
  try {
    ...
    if (audit.vulnerabilities) {
      for (const [pkg, vulnData] of Object.entries(audit.vulnerabilities as any)) {
        const data = vulnData as any;
        const severity = data.severity as 'critical' | 'high' | 'medium' | 'low';
...
async function scanCodeSecurity(cwd: string): Promise<SecurityIssue[]> {
  const issues: SecurityIssue[] = [];
...
async function scanDirectorySecurity(dir: string, issues: SecurityIssue[]): Promise<void> {
  try {
...
async function scanFileSecurity(file: string, issues: SecurityIssue[]): Promise<void> {
...
async function scanEnvSecurity(cwd: string): Promise<SecurityIssue[]> {
  const issues: SecurityIssue[] = [];
...
async function scanApiSecurity(cwd: string): Promise<SecurityIssue[]> {
  const issues: SecurityIssue[] = [];
...
async function scanApiRoutes(dir: string, issues: SecurityIssue[]): Promise<void> {
  try {
    const entries = await fs.readdir(dir, { withFileTypes: true });
...
function generateSecurityReport(issues: SecurityIssue[], blockOnCritical: boolean): string {
...
function generateDetailedSecurityReport(issues: SecurityIssue[]): string {
